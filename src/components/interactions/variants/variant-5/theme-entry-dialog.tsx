...
type ThemeEntryDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  title: string;
  description: string;
  colors: ColumnColor[];
  existingEntries?: ThemeEntry[];
  saveLabel: string;
  onSave: (entries: ThemeEntry[]) => void;
  requireAtLeastOne?: boolean;
};

export function ThemeEntryDialog({
  open,
  onOpenChange,
  title,
  description,
  ...
  function handleSave() {
    ...
    if (requireAtLeastOne && entries.length === 0) return;
          ...
          {colors.map((color) => (
            <div key={color.id} className="flex items-center gap-3">
              ...
              <Input
                ...
                placeholder={color.id === "default" ? "None" : color.label}
                onChange={(e) =>
